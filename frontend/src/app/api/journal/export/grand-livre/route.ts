/**
 * Next.js proxy — GET /api/journal/export/grand-livre
 * Forwards to Django GET /api/v1/journal/export/grand-livre/
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  const djangoRes = await fetch(
    `${DJANGO_URL}/api/v1/journal/export/grand-livre/${qs ? `?${qs}` : ""}`,
  ...
  if (req.nextUrl.searchParams.get("format") === "csv") {
    ...
    const disposition = djangoRes.headers.get("Content-Disposition") ?? 'attachment; filename="grand_livre.csv"';
